...
> 배달 · 밀키트 · 직접 조리의 실제 비용을 비교하여 자취생의 합리적인 식습관 형성을 돕는 서비스
...
**설계 의도:** 선택된 카테고리에 따라 Recipe API 및 Virtual Mart DB에서 가용 메뉴를 실시간으로 바인딩한다. MenuType을 통해 불필요한 분석 과정을 생략하고, 실제로 가능한 옵션만 노출하여 UX 효율을 높였다.
...
선택한 메뉴(예: 김치찌개·한식)에 대해 **세 가지 방식의 비용 파라미터**를 수집하는 화면.  
단순 가격 비교를 넘어 **기회비용(Opportunity Cost)** 까지 산출하기 위한 핵심 단계다.

| 섹션 | 주요 입력 항목 | 비고 |
|------|--------------|------|
| 🔴 배달 정보 | 배달 음식 가격, 배달팁, 최소주문금액 | 배달팁은 구독 서비스(배민클럽 등) 고려해 가변 인자로 처리 |
| 🟡 밀키트 정보 | 밀키트 가격, 조리 시간 | 가격은 API로 실시간 최저가 자동 로드 |
| 🟢 직접 조리 정보 | 조리 시간, 설거지 시간, 사용 도구 선택 | 조리 시간은 Recipe API 자동 입력 |
...
<img src="./images/07_detail.png" width="250">

분석 결과의 신뢰도를 높이기 위해 **비용 산출 공식과 항목별 근거를 공개**하는 화면이다.

**비용 산출 공식:**

$$C_{Total} = C_{Food} + C_{Opportunity} + C_{Labor} + C_{Tools}$$

**🔴 배달**
- 음식 가격 9,000원 + 배달팁 0원 + 최소주문금액 보정 1,000원 = **10,000원**

**🟡 밀키트**
- 밀키트 가격 5,499원 + 기회비용 688원 *(조리시간 × 0.2 × 최저임금)* = **6,187원**

**🟢 직접 조리**
- 식재료비 2,800원 + 기회비용 688원 + 가사노동비 344원 *(설거지 시간 × 최저임금)* + 도구 비용 300원 = **4,132원**

> *2026-05-07 기준 가격 / 최저임금 10,320원 기준*

---

## 8. Hidden Opportunity Report (절약 리포트)
...
- **분석 내역 History 로그** — 날짜별 소비 패턴 회고 가능
  - 예: 김치찌개 / 2026-05-07 · 직접 조리 선택 → **+5,868원 절약** / 4,132원 지출
