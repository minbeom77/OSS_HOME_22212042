...
classDiagram
    ...
    class User["User"] {
        +String userId
        +loginStatus, savingLog
    }
    class Menu["Menu"] {
        +String menuName
        +category, menuType
    }
    class VirtualMartDB["VirtualMartDB"] {
        +String menuName
        +foodPrice, minOrder
    }
    class RecipeService["RecipeService"] {
        +fetchRecipe(menuName)
    }
    class IngredientMapper["IngredientMapper"] {
        +normalize(rawName)
    }
    class ShoppingService["ShoppingService"] {
        +fetchPrice(name)
    }
    class CostCalculator["CostCalculator"] {
        +calcDelivery() / calcMealKit()
        +calcCooking() / getBestOption()
    }
    class AnalysisLog["AnalysisLog"] {
        +Date date
        +menu, saving, choice
    }
    ...
    User --> Menu : selects
    User --> VirtualMartDB : uses
    
    Menu --> RecipeService : calls
    
    RecipeService ..> IngredientMapper
    RecipeService ..> CostCalculator
    
    VirtualMartDB --> ShoppingService
    
    IngredientMapper ..> ShoppingService
    IngredientMapper ..> CostCalculator
    
    ShoppingService ..> CostCalculator
    
    CostCalculator --> AnalysisLog : saves
    CostCalculator --> ReportGenerator : triggers

    style User fill:#FDF9F3,stroke:#B07A29,stroke-width:2px
    style Menu fill:#FDF9F3,stroke:#B07A29,stroke-width:2px
    style VirtualMartDB fill:#FDF9F3,stroke:#B07A29,stroke-width:2px
    style RecipeService fill:#FDF9F3,stroke:#B07A29,stroke-width:2px
    style IngredientMapper fill:#FDF9F3,stroke:#B07A29,stroke-width:2px
    style ShoppingService fill:#FDF9F3,stroke:#B07A29,stroke-width:2px
    style CostCalculator fill:#FDF9F3,stroke:#B07A29,stroke-width:2px
    style AnalysisLog fill:#FDF9F3,stroke:#B07A29,stroke-width:2px
    style ReportGenerator fill:#FDF9F3,stroke:#B07A29,stroke-width:2px
