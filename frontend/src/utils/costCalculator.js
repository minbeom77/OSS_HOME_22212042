...
export function calcCostsLocal(menu, input) {
  const { deliveryFee, foodPrice, minOrder, kitPrice, kitMin,
          cookMin, laborMin, toolCost } = input; 

  const deliveryCost = foodPrice + deliveryFee + Math.max(0, minOrder - foodPrice); 


  const mType = menu.menuType || menu.type; 
  ...
  const cookingCost = mType === "ALL" 
    ? menu.ingredientCost + (cookMin / 60) * 0.2 * MIN_WAGE
      + (laborMin / 60) * 0.2 * MIN_WAGE + toolCost 
    : null; //
