...
export default function ResultScreen({ cat, menu, costs, onNext, onBack }) {
  ...
  useEffect(() => {
    setSelectedOption(best || "delivery");
  }, [costs, best]);

  const opts = [
    { key: "delivery", label: "배달 주문", cost: delivery || 0, icon: "🛵", color: "bg-[#D85A30]", textColor: "text-[#D85A30]", borderColor: "border-[#D85A30]", lightBg: "bg-[#FAECE7]", ringColor: "ring-[#F5C4B0]" },
  ];
  
  if (mealkit !== null && mealkit !== undefined) {
    opts.push({ key: "mealkit", label: "밀키트 조리", cost: mealkit, icon: "📦", color: "bg-[#4A7047]", textColor: "text-[#4A7047]", borderColor: "border-[#4A7047]", lightBg: "bg-[#E9F0E8]", ringColor: "ring-[#C6DCBF]" });
  }
  if (cooking !== null && cooking !== undefined) {
    opts.push({ key: "cooking", label: "직접 요리", cost: cooking, icon: "🍳", color: "bg-[#BA7517]", textColor: "text-[#BA7517]", borderColor: "border-[#BA7517]", lightBg: "bg-[#FAEDCD]", ringColor: "ring-[#F0D4B0]" });
  }
  ...
  return (
    <div className="max-w-xl mx-auto px-6 py-8">
          ...
          return (
            <div
              ...
              className={`relative rounded-2xl p-5 border-2 transition-all duration-300 cursor-pointer select-none shadow-sm
                ${isSelected ? `${o.borderColor} ${o.lightBg} scale-[1.01] ring-4 ${o.ringColor}/50` : "bg-white border-[#E9E4D9] hover:border-[#B0A996]"}
              `}
            >
              {isBest && (
                <span className={`absolute top-3 right-4 text-[10px] ${o.lightBg} ${o.textColor} font-black px-2 py-0.5 rounded-full tracking-wide`}>★ 최고 대안 추천</span>
              )}
              ...
              <div className={`h-2 ${isSelected ? 'bg-white/60' : o.lightBg} rounded-full overflow-hidden`}>
                <div className={`h-full rounded-full transition-all duration-700 ${o.color}`} style={{ width: `${(o.cost / maxCost) * 100}%` }} />
              </div>
            </div>
          );
        })}
      ...
      {breakdown && breakdown[selectedOptData.key] && (
        <div className="mb-8">
          ...
          <div className={`rounded-2xl p-5 space-y-3 transition-colors duration-300 border ${selectedOptData.lightBg} ${selectedOptData.borderColor} border-opacity-40 shadow-inner`}>
            {breakdown[selectedOptData.key].map((item, idx) => (
              <div key={idx} className="flex justify-between text-xs my-1">
                <span className="text-[#7A7466]">{item?.label?.includes("최소주문금액") ? "최소주문 미달 분 부족 금액" : item?.label || "비용 항목"}</span>
            ...
            <div className="flex justify-between items-center mt-3 pt-3 border-t border-black/10">
              ...
              <span className={`text-sm font-black ${selectedOptData.textColor}`}>
                {selectedOptData.cost.toLocaleString()}원
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between gap-4 pt-5 border-t border-[#E9E4D9]">
        <Button variant="secondary" onClick={onBack} className="flex-1 py-3.5 font-bold text-sm rounded-xl border-[#D3D1C7] text-[#5C584E] hover:bg-[#F1EFE8] transition-all text-center">
          ← 다시 분석
        </Button>
        <Button onClick={() => onNext(selectedOptData.key)} className="flex-1 py-3.5 font-bold text-sm rounded-xl bg-[#BA7517] hover:bg-[#8E5912] text-white transition-all shadow-md text-center">
